...
- (void)viewDidLoad {
    ...
    BOOL isExpire = [Tools checkTimeExpire:@"DISCOVER_LASTUPDATE" ExpireSeconds:3600];
    NSLog(@"isExpire %d", isExpire);
    
    // 从Cache中载入数据
    NSString* jsonStr = [Tools getCacheJson:@"DISCOVER_JSON"];
    ...
    {
        NSLog(@"Should Request discover info");
        [self loadData];
    }
    [self loadData];
}
...
{
    //TODO 更新数据库中缓存的数据和时间
    BOOL ret = [Tools setUpdateTime:@"DISCOVER_LASTUPDATE"];
    ...
    ret =[Tools setCacheJson: serialStr forKey:@"DISCOVER_JSON"];
...
-(void) parseData:(NSDictionary*) result{
    ...
    for (NSInteger i=0; i<arrary.count; i++) {
        ...
        if ([show_type isEqualToString:@"轮播"]) {
            ...
            self.lunboUrlArray = [[NSMutableArray alloc]init];
            for (NSDictionary *itemDic in slider) {
                ...
                [self.lunboUrlArray addObject:lunboModel.open_url];
...
- (void)loadData{
//    [GiFHUD setGifWithImageName:@"jindu.gif"];
//    [GiFHUD show];
   ...
   [MyDataService requestAFURLPlus:@"/res/discover?app_ver=1.0&json_ver=201503100932&lng=&lat=" httpMethod:@"POST" params:childDic data:nil completion:^(id result) {
//       [GiFHUD dismiss];
...
-(void)EScrollerViewDidClicked:(NSUInteger)index andPages:(NSInteger)pages;{
    index = (index+1)%self.lunboUrlArray.count;
    //NSLog(@"lunbo %d %@",index,[self.lunboUrlArray objectAtIndex:index]);
    ...
    webController.destUrl = [self.lunboUrlArray objectAtIndex:index];
